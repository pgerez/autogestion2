...
EOF;
            ...
            $html = <<<EOF
                    ...
                    <br>
                    <span style="text-justify: auto">
                    Recibimos de {$recibo->getObrasocial()} la suma de <strong>{$this->numtoletras((string) $recibo->getMonto())}</strong> en oncepto de pago de las siguientes facturas: <br>{$recibo->getCuota()->getFacturas()}
                    </span>
                    <br><br>
                        Cheque/Transferencia:<strong> {$recibo->getCuota()->getNumeroComprobante()}</strong><br>
                        Cuenta/Banco: <strong>Santiago del Estero</strong><br>  
