$( document ).ready(function() {
	...
	const parseResult = _text => {
	  ...
	  if( validarFecha(fechaNac) &&
		...
		//$('#dni').val(dni);
		$('#filter_personaid__dni_value').val(dni);
		//$('#nombre_form').val(nombre + ' ' + apellido);		
	...
	const mainLoop = () => {
	  ...
	  var _element = document.getElementById('foco');
	  
	  let _x = getOffset(_element).left;
	  let _y = getOffset(_element).top;
	  
	  //ctx.drawImage(video, 0, 0, _canvasWidth, video.videoHeight*_canvasWidth/video.videoWidth);
	  //ctx.drawImage(video, _x, _y, _element.offsetWidth, _element.offsetHeight, _X, _Y, _element.offsetWidth, _element.offsetHeight);

	  ctx.drawImage(image, _x, _y - _element.offsetHeight - 30, _element.offsetWidth + 45, _element.offsetHeight, 0, 0, image.offsetWidth, _element.offsetHeight);
